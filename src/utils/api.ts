...
export const gameAPI = {
  ...
  getGame: (gameId: string) => api.get('/match/snapshot').then(response => ({
    data: {
      game: {
        gameSession: {
          id: gameId,
        ...
        agents: (response.data.bots || []).map((botId: string, index: number) => ({
          id: botId,
  ...
  createGame: (name: string, maxParticipants?: number, duration?: number) =>
  ...
  startGame: (gameId: string) => api.post('/match/resume'),

  // 暂停游戏 - 使用perp-bot-mvp的pause接口
  pauseGame: (gameId: string) => api.post('/match/pause'),

  // 重置游戏 - 使用perp-bot-mvp的reset接口
  resetGame: (gameId: string) => api.post('/match/reset'),

  // 获取游戏消息 (模拟数据)
  getMessages: (gameId: string) => Promise.resolve({ data: [] }),

  // 获取市场历史数据 (模拟数据)
  getMarketHistory: (gameId: string) => Promise.resolve({ data: [] })
}
